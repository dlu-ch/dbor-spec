...
The byte sequence \ByteSequence{\DborFirstByte\DborNumberValueColour{38}, \DborNextByte{4B}} "is a" \DborNumberValue{},
...
\subsection{Elementary values (\DborElementaryValue, \DborNumberValue, \DborNumberlikeValue, \DborStringValue)}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\footnote{%
    This reflects the idea of a negative infinitesimal (\href{https://en.wikipedia.org/wiki/Hyperreal_number}{hyperreal}
    or \href{https://en.wikipedia.org/wiki/Surreal_number}{surreal}) number as members of an extension field
    of $\SetOfReals$.
    ...
    $-0$ and $0$ are treated as a negative or positive infinitesimal numbers in this operation.
}
...
it is \DborIntegerToken*($3$, $m$) {\Concat} \ByteSequence{b_{n - m + 1}, \ldots, b_n} for an appropriate
$m \in \IntegerInterval{0}{N + 23}$.
It is a \emph{well-formed} \DborUtfEightStringValue{} if and only if it is a size-correct \DborUtfEightStringValue{} and
\ByteSequence{b_{n - m + 1}, \ldots, b_n} is empty or starts with a well-formed UTF-8 encoded Unicode code point.
...
The canonical value of \DborUtfEightStringValue(\ByteSequence{b_1, \ldots, b_m}) is
...
\DborIntegerToken*($4$, $m$) {\Concat} \ByteSequence{b_{n - m + 1}, \ldots, b_n} for an appropriate
$m \in \IntegerInterval{0}{N + 23}$.
...
if and only if it is
\DborIntegerToken*($5$, $m$) {\Concat} \ByteSequence{b_{n - m + 1}, \ldots, b_n} for an appropriate
$m \in \IntegerInterval{0}{N + 23}$.
...
it begins with \DborNaturalToken*($6$, $0$, $m$) for an appropriate $m \in \IntegerInterval{0}{N + 23}$
